...
class TestBooksCollector:
    ...
    def test_add_new_book_not_add_double(self):
        collector = BooksCollector()
        collector.add_new_book('Гордость и предубеждение и зомби')
        collector.add_new_book('Гордость и предубеждение и зомби')
        assert len(collector.books_genre) == 1

    def test_set_book_genre_not_sets_genre(self):
        collector = BooksCollector()
        collector.set_book_genre('1+1','Комедии')
        assert collector.get_book_genre('1+1') is None

    def test_get_book_genre_adding_book_to_genre(self):
        collector = BooksCollector()
    ...
    def test_get_books_with_specific_genre_not_genre_empty_list(self):
        collector = BooksCollector()
        collector.add_new_book('Собака Баскервилей')
        assert collector.get_books_with_specific_genre('Мемуары') == []

    def test_get_books_with_specific_genre_not_book_empty_list(self):
        collector = BooksCollector()
    ...
    def test_get_books_for_children_returns_allowed_book(self):
        collector = BooksCollector()
